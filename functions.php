...
function process_new_task() {
    // Verify nonce
    check_ajax_referer( 'new_task_nonce', 'security' );

    // Sanitize and store the form data
    $task_name = sanitize_text_field( $_POST['task_name'] );
    $task_location = sanitize_text_field( $_POST['task_location'] );
    $task_url = filter_var( $_POST['task_url'], FILTER_VALIDATE_URL );
    $task_status = sanitize_key( $_POST['task_status'] );
    $task_priority = sanitize_key( $_POST['task_priority'] );
    $task_duedate = sanitize_text_field( $_POST['task_duedate'] );
...
//for done delete
add_action('wp_ajax_delete_done_todo', 'delete_done_todo');
function delete_done_todo(){
...
//for update
add_action('wp_ajax_update_todo', 'update_todo_callback');

function update_todo_callback() {
  ...
  if($result == TRUE){
	header('location:/site2/todo');
}
else{
	echo "Error:" . $result . "<br>" ;
}
     
    wp_send_json_success('Todo updated successfully');
}
...
add_action('wp_ajax_delete_todo', 'delete_todo_callback');
function delete_todo_callback() {
    ...
    if ($todo_id <= 0) {
     ...
     $result = $wpdb->delete($table_name, array('id' => $todo_id));

    if ($result !== false) {
        wp_send_json_success('Todo item deleted successfully');
    } else {
        wp_send_json_error('Failed to delete todo item');
    }
}
//for submit and add 

add_action('wp_ajax_add_todo', 'add_todo_callback');
add_action('wp_ajax_nopriv_add_todo', 'add_todo_callback');

function add_todo_callback() {
    global $wpdb;
...
function enqueue_custom_scripts() {
    wp_enqueue_script('todo-script', get_stylesheet_directory_uri() . '/assets/js/todo.js', array('jquery'), null, true);
}

add_action('wp_enqueue_scripts', 'enqueue_custom_scripts');

function enqueue_custom_styles() {
    wp_enqueue_style( 'custom-styles', get_stylesheet_directory_uri() .'/assets/css/todo.css' );
}
add_action( 'wp_enqueue_scripts', 'enqueue_custom_styles' );

require_once get_template_directory() . '/todo-ajax.php';
require_once get_template_directory(). '/todo-auto-sendSMS.php';
